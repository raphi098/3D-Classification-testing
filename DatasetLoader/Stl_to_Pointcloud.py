...
class StlToPointCloud:
    def __init__(self, dataset_path, number_of_points, train_test_split=0.8):
        ...
        self.pc_dataset_path = os.path.join("Data_prepared",f"{os.path.basename(self.dataset_path)}_{1024}_points")
        self.folder_names = []
        ...
        for folder in tqdm(self.folder_names, desc="Processing folders"):
            ...
            for split in ["train", "test"]:
                ...
                for file in tqdm(files, desc=f"Processing files in {split}", leave=False):
                    ...
                    destination_path = os.path.join(self.pc_dataset_path, folder.replace(" ", "_"), split, file.replace(".stl", ".pcd"))
